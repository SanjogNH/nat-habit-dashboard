...
function refreshSubcategoryOptions() {
  // Cascade Cat → Subcat: instead of replacing the whole widget (which would
  // wipe the user's current subcategory selection), call setOptions on the
  // existing widget. setOptions clamps the selection to the new option set
  // and returns the dropped values so we can tell the user what disappeared.
  const { subF } = LocalState.filters;
  const newOpts = collectSubcategoryOptions(LocalState.categories);
  const dropped = subF.setOptions(newOpts);
  LocalState.subcategories = subF.getSelected();
  if (dropped && dropped.length) {
    const sample = dropped.slice(0, 3).join(", ");
    const extra = dropped.length > 3 ? ` and ${dropped.length - 3} more` : "";
    toast(`Removed ${dropped.length} sub-categor${dropped.length === 1 ? "y" : "ies"} not in selected categories: ${sample}${extra}`);
  }
  // SKU picker rebuilds via rerender() → renderSkuPicker(); any SKUs that no
  // longer match the new cat/subcat scope get clamped there. The SKU widget
  // is re-created each rerender so a separate toast would fire mid-flight;
  // we rely on the implicit clamp without a notification here.
}
...
function rerender() {
  ...
  // Paint active-filter chips above each table on this tab.
  renderChipsAcrossTab(document.getElementById("content-impressions"), buildChips());
}

/* ---------------------------------------------------------------- *
 * Active-filter chip spec
 *
 * Surfaces narrowed filters above each table; × resets that filter to
 * "all" / default.
 * ---------------------------------------------------------------- */
function buildChips() {
  const chips = [];
  ...
  // Platforms — "all" = the impressions-platform list we built in buildFilters.
  const impPlatforms = new Set();
  for (const r of State.data?.weekly_sfr       || []) if (r.platform) impPlatforms.add(r.platform);
  for (const r of State.data?.weekly_catalogue || []) if (r.platform) impPlatforms.add(r.platform);
  const allPlats = [...impPlatforms].sort();
  ...
  // Categories — "all" comes from the metadata.
  if (catsF) {
  ...
  // Subcategories — driven by the cascading category list.
  if (subF) {
    const allSubs = collectSubcategoryOptions(LocalState.categories).map(o => o.value);
    const subChip = buildMultiSelectChip(subF, "Sub-categories",
      () => allSubs, { resetTo: allSubs });
    if (subChip) chips.push(subChip);
  }
