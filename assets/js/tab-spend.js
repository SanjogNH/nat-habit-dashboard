...
function rerender() {
  ...
  renderChipsAcrossTab(document.getElementById("content-spend"), buildChips());
}
...
function buildChips() {
  ...
  // Platforms — for Spend, "all" is the spend-platforms set we computed in
  // buildFilters (only those with bcg_spend rows).
  const spendPlats = [...new Set((State.data?.bcg_spend || [])
    .map(r => r.platform).filter(Boolean))].sort();
  const platChip = buildMultiSelectChip(platsF, "Platforms",
    () => spendPlats, { resetTo: spendPlats });
  ...
  const levelChip = buildSegmentChip(levelF, "View level", "overall",
    { overall: "Overall", category: "Category",
      subcategory: "Sub-category", channel: "Marketing Channel" });
  ...
  if (LocalState.level !== "overall" && _dimPicker) {
    ...
    if (!isAll) {
      ...
      const shown = sel.slice(0, 3).join(", ");
