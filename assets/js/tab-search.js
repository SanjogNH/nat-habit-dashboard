    ...
    <div class="modal-overlay" id="search-modal" hidden>
      <div class="modal-card">
        ...
        <div class="modal-body">
          <div class="filter-chips" id="search-modal-chips" hidden></div>
...
function buildFilters() {
  ...
  categoriesF.onChange(() => {
    ...
    // dropping picks that no longer apply. setOptions returns the dropped
    // keyword strings so we can tell the user what disappeared.
    const dropped = keywordsF.setOptions(collectKeywordOptions(LocalState.globalCategories));
    LocalState.globalKeywords = keywordsF.getSelected();
    if (dropped && dropped.length) {
      const sample = dropped.slice(0, 3).join(", ");
      const extra = dropped.length > 3 ? ` and ${dropped.length - 3} more` : "";
      toast(`Removed ${dropped.length} keyword${dropped.length === 1 ? "" : "s"} not in selected categories: ${sample}${extra}`);
    }
    rerender();
  });
...
function rerender() {
  ...
  // Active-filter chips above every table in the tab. The modal also gets
  // chips when open — that's wired separately in openModal so chips re-paint
  // when the user opens it.
  renderChipsAcrossTab(document.getElementById("content-search"), buildChips());
  // Re-paint chips inside the modal if it's currently visible.
  const modal = document.getElementById("search-modal");
  if (modal && !modal.hidden) {
    renderFilterChips(document.getElementById("search-modal-chips"), buildChips());
  }
}
 ...
 *
 * Each chip surfaces a narrowed filter (one a "default/all" view wouldn't show)
 * and its × click resets that filter to "all" / default. Tabs call this from
 * their rerender path, so the chip row updates in lockstep with the filter
 * bar.
 * ---------------------------------------------------------------- */
function buildChips() {
  const chips = [];
  if (!LocalState.filters) return chips;
  const { dateF, viewF, platsF, brandedF, metricF, categoriesF, keywordsF }
    = LocalState.filters;

  // Date range — always informative; chip only when narrowed.
  const dateChip = buildDateChip(dateF);
  if (dateChip) chips.push(dateChip);

  // Granularity (only chip when non-default).
  const viewChip = buildSegmentChip(viewF, "View", "weekly",
    { weekly: "Weekly", daily: "Daily" });
  if (viewChip) chips.push(viewChip);

  // Platforms — chip when a subset is selected.
  const platformOpts = LocalState._allPlatformOptions || [];
  // In Rank mode, the picker is constrained to rank-capable platforms; use
  // that as the "all" set so the chip doesn't spuriously appear.
  const platformAll = (LocalState.metricMode === "rank" && LocalState.view === "weekly")
    ? (LocalState._rankPlatformOptions || []).map(o => o.value)
    : platformOpts.map(o => o.value);
  const platChip = buildMultiSelectChip(platsF, "Platforms",
    () => platformAll, { resetTo: platformAll });
  if (platChip) chips.push(platChip);

  // Branded / Generic — chip when not "both".
  const brChip = buildSegmentChip(brandedF, "Keyword type", "both",
    { both: "Both", branded: "Branded", generic: "Generic" });
  if (brChip) chips.push(brChip);

  // Metric mode (Rank vs Volume) — only when non-default and the toggle is
  // actually visible (Weekly view with any rank-capable platform).
  const metricVisible = LocalState.view === "weekly"
    && (LocalState._rankPlatformOptions || []).length > 0;
  if (metricVisible) {
    const mChip = buildSegmentChip(metricF, "Metric", "rank",
      { rank: "Rank", volume: "Volume" });
    if (mChip) chips.push(mChip);
  }

  // Categories.
  if (categoriesF) {
    const allCats = (State.data?.metadata?.categories || []);
    const catChip = buildMultiSelectChip(categoriesF, "Categories",
      () => allCats, { resetTo: allCats });
    if (catChip) chips.push(catChip);
  }

  // Keywords (global tab-wide filter — distinct from the trend-chart picker).
  if (keywordsF) {
    const allKw = keywordsF.getSelected; // we don't have a clean "all keywords"
    // accessor; use the widget's own option count via getSelected as a proxy.
    // setOptions clamps to options, so resetting to [] gives "no filter" =
    // all keywords per the setOrNullForAll convention used in aggregations.
    const kwSel = keywordsF.getSelected();
    if (kwSel.length > 0) {
      chips.push({
        label: `Keywords: ${kwSel.slice(0, 3).join(", ")}${kwSel.length > 3 ? ` +${kwSel.length - 3}` : ""}`,
        onClear: () => keywordsF.setSelected?.([]),
      });
    }
  }
...
function renderTopTable(agg, rankMode) {
  ...
  // after sort); every other column is. The leading three columns (#, Keyword,
  // Category) carry sticky-col-{1,2,3} classes so they stay visible when the
  // table scrolls horizontally through periods.
  let head = `<tr>
    <th class="rank sticky-col-1">#</th>
    ${sortableTh("keyword",  "Keyword",  sortSpec, "left", null, "sticky-col-2")}
    ${sortableTh("category", "Category", sortSpec, "left", null, "sticky-col-3")}`;
 ...
 * `extraClass` is appended to the className (used for sticky-col-N on the
 * leading frozen columns).
 */
function sortableTh(colKey, label, sortSpec, align, tooltip, extraClass) {
  const cls = align === "num" ? "num" : "";
  const active = sortSpec.col === colKey;
  const arrow = active ? (sortSpec.dir === "asc" ? " ▲" : " ▼") : "";
  const cn = `${cls} sortable${active ? " is-active-sort" : ""}${extraClass ? " " + extraClass : ""}`.trim();
...
function rowHtml(entry, rank, showPeriods, rankMode) {
  let row = `<td class="rank sticky-col-1">${rank}</td>`;
  row += `<td class="kw-cell sticky-col-2">${escapeHtml(entry.keyword || "—")}</td>`;
  row += `<td class="cat-cell sticky-col-3">${escapeHtml(entry.category || "—")}</td>`;
  for (const p of showPeriods) {
...
function openModal() {
  ...
  let head = `<tr><th class="rank sticky-col-1">#</th><th class="sticky-col-2">Keyword</th><th class="sticky-col-3">Category</th>`;
  for (const p of showPeriods) {
    const isLatest = (p === agg.latestPeriod);
    const meta = agg.periodMeta.get(p);
  ...
  // Sync chips into the modal body so filter context is visible there too.
  renderFilterChips(document.getElementById("search-modal-chips"), buildChips());
}
function closeModal() {
